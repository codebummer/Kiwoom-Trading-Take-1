...
class Kiwoom(QAxWidget):
    ...
    def _realtype_stock_status(self, code):
        ...
        self._real_data_to_sql('주식시세', f'주식시세_{code}.db', pd.DataFrame(add))

    #     df_name, df = self._df_generator('주식시세', code, add)
    #     self._real_data_to_sql('주식시세', df_name, df)
    
    # def _df_generator(self, realtype, stockcode, data):
    #     df_name = realtype+'_'+stockcode
    #     self.real_tr_stocks.add(df_name)
    #     if df_name in self.real_tr_stocks:
    #         exec(f'return {df_name}, {df_name}.append({data})')
    #     else:
    #         exec(f'return {df_name}, pd.DataFrame({data})')
    ...
    def _realtype_stock_made(self, code): 
        ...
        self._real_data_to_sql('주식체결', f'주식체결_{code}.db', pd.DataFrame(add))
    ...
    def _realtype_order_made(self, code):
        ...
        self._real_data_to_sql('주문체결', f'주문체결_{code}.db', pd.DataFrame(add))
