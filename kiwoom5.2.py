...
class Kiwoom(QAxWidget):
        ...
        for renew, call in renew_list.items():
            for df_name in self.tr_data['charts'].keys():
                ...
                if renew in df_name:
                    ...
                    if breakcount == len(self.orders['follow'].keys()):
                        breakcount = 0
                        break
    ...
    def set_order(self, rqname, scrno, accno, ordertype, code, qty, price, hogagb, orgorderno):
        ...
        # if ordertype == '1':
        #     self._follow_stocks(stock, 'follow', 0)
        #     self.orders['orders']['buying'].add([self.all_stocks['tickerkeys'][code]])
        # elif ordertype == '2':
        #     self.orders['orders']['selling'].add([self.all_stocks['tickerkeys'][code]])
        # elif ordertype == '3':
        #     self.orders['orders']['buying'].remove(self.all_stocks['tickerkeys'][code])
        # elif ordertype == '4':
        #     self.orders['orders']['buying'].remove(self.all_stocks['tickerkeys'][code])            
    ...
    def _find_buy_sell(self):
        ...
        for df_name in self.tr_data['charts'].keys():
            if '월봉' in df_name or '주봉' in df_name or '일봉' in df_name or '분봉' in df_name:
                _mas(df_name)
    ...
    def _find_fast_buy_sell(self, ISMA):
        buy_conditions = {}
        sell_conditions = {}
    ...
    def _order_strategies(self):
        ...
        for stock, details in self.orders.items():
            for status, values in details.items():
                if status == 'buying' and values == 'qued':
                    self.buy(stock)
                    break
                elif status == 'selling' and values == 'qued':
                    self.sell(stock)
                    break
            
                
            
            # for order in list(self.orders['orders']['buy']):
            #     self.buy(order)
                
            #     self.orders['orders']['buying'].remove(order)
            # for order in list(self.orders['orders']['sell']):
            #     self.sell(order)   
            #     self.orders['orders']['sell'].remove(order)
    
    def _auto_orders(self):
        ISMA = False
        self._find_fast_buy_sell(ISMA)
        ISMA = True
    ...
    def onestop_stock(self, *stocks):
        ...
        # self._apply_strategies(self.tr_data['charts'].keys())
        self._apply_fast_strategies(self.tr_data['charts'].keys())
    ...
    def onestop_volitility(self):
        ...
        self.get_balance()
        if self.tr_data['noncharts']['예수금상세현황']['주문가능금액'] < 1_000_000*self.tax_fee_ratio:
            print('Not enough money to invest. Terminating the trading.')           
        
        while True:
            orders = self._find_fast_buy_sell()
